...
class LCDownloader(LCDataRetriever):
    def __init__(self, cookie, path="./", target_folder="Markdown"):
    ...
    def write_to_plain_md(self, entry):
        base_url = "https://leetcode.com/problems/%s/"
        title = entry['title']
        question_info = self.retrieve_question(self.naive_title_slug(title))
        question_id = question_info['questionFrontendId']
        filename = os.path.join(self.download_path, f"{title}.md")

        new_related_template = "- %(related_title)s (%(related_difficulty)s) "

        new_tag_template = "**_%(tag)s_** "

        tags = []
        for tag in question_info['topicTags']:
            tags.append(new_tag_template % {
                "tag": tag['name']
            })
        ...
        stats = json.loads(question_info['stats'])
        for r in rel:
            related.append(new_related_template % {
                "related_title": r["title"],
                "related_difficulty": r["difficulty"]
            })

        with open(filename, "w") as f:
            f.write(raw_md_template % {
                "id": question_id,
                "title": title,
                "difficulty": question_info['difficulty'],
                "small_tags": '&nbsp; • &nbsp;'.join(tags),
